...
function hookResponse(status = 200, overrides: Record<string, unknown> = {}) {
  return new Response(JSON.stringify({
    ...
    writeCount: 2,
    results: [{ policy: 'health.visitor', layer: 'visitor', allowed: status === 200 }],
    ...overrides,
...
describe('shared rate limiter core', () => {
  ...
  it('HMACs identities and creates an internal decision id unrelated to requestId', async () => {
    ...
    const result = await acquireSharedRateLimit(request, context, ['health.visitor'], { fetchImpl });
  ...
  it('fails closed in enforce and allows backend failure only in shadow', async () => {
    ...
    const enforced = await acquireSharedRateLimit(request, context, ['health.visitor'], { fetchImpl });
    ...
    const shadow = await acquireSharedRateLimit(request, context, ['health.visitor'], { fetchImpl });
